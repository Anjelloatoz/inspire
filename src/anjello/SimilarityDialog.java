...
public class SimilarityDialog extends JDialog {
	...
	SimilConfigPanel clientType;
	SimilConfigPanel clientExperience;
	SimilConfigPanel buildingType;
	SimilConfigPanel buildType;
	SimilConfigPanel costParameter;
	SimilConfigPanel timeParameter;
	...
	private void configureFrame()
	{
		...
		this.setTitle("Configure Similarity - Prototype developed and owned by Anjello - Anjelloatoz@gmail.com");
...
//		this.getContentPane().add(image, BorderLayout.WEST);
		...
		panel.add(new JLabel("ClientType"));
		panel.add(clientType = new SimilConfigPanel(stringfunctions));
		
		panel.add(new JLabel("Client Experience"));
		panel.add(clientExperience = new SimilConfigPanel(stringfunctions));
		
		panel.add(new JLabel("Building Type"));
		panel.add(buildingType = new SimilConfigPanel(ontofunctions));
		
		panel.add(new JLabel("Build Type"));
		panel.add(buildType = new SimilConfigPanel(stringfunctions));
		
		panel.add(new JLabel("Cost Parameter"));
		panel.add(costParameter = new SimilConfigPanel(stringfunctions));
		
		panel.add(new JLabel("Time Parameter"));
		panel.add(timeParameter = new SimilConfigPanel(enumfunctions));
...
//		Lay out the panel.
	...
	public NNConfig getSimilarityConfig()
	{
		...
		similConfig = clientType;
		attribute = new Attribute("ClientType",ConstructionDescription.class);
		...
		similConfig = this.clientExperience;
		attribute = new Attribute("ClientExperience",ConstructionDescription.class);
		function = localSimilFactory(similConfig.getSimilFuntion(), similConfig.getParam()); 
		config.addMapping(attribute, function);
		config.setWeight(attribute, similConfig.getWeight());

		similConfig = this.buildingType;
		attribute = new Attribute("BuildingType",ConstructionDescription.class);
		function = localSimilFactory(similConfig.getSimilFuntion(), similConfig.getParam()); 
		config.addMapping(attribute, function);
		config.setWeight(attribute, similConfig.getWeight());

		similConfig = this.buildType;
		attribute = new Attribute("BuildType",ConstructionDescription.class);
		function = localSimilFactory(similConfig.getSimilFuntion(), similConfig.getParam()); 
		config.addMapping(attribute, function);
		config.setWeight(attribute, similConfig.getWeight());

		similConfig = costParameter;
		attribute = new Attribute("CostParameter",ConstructionDescription.class);
		function = localSimilFactory(similConfig.getSimilFuntion(), similConfig.getParam()); 
		config.addMapping(attribute, function);
		config.setWeight(attribute, similConfig.getWeight());

		similConfig = this.timeParameter;
		attribute = new Attribute("TimeParameter",ConstructionDescription.class);
		function = localSimilFactory(similConfig.getSimilFuntion(), similConfig.getParam()); 
		config.addMapping(attribute, function);
		config.setWeight(attribute, similConfig.getWeight());
